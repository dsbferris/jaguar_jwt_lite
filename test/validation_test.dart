  ...
  group('Validation', () {
    ...
    group('Issuer', () {
      ...
      test('Issuer mismatch', () {
        ...
        expect(
          () => claimSetIssuer0.validate(issuer: wrongIssuer),
          throwsA(equals(JwtException.incorrectIssuer)),
        );

        expect(
          () => claimSetIssuer1.validate(issuer: wrongIssuer),
          throwsA(equals(JwtException.incorrectIssuer)),
        );
      });
    });
    ...
    group('Audience', () {
      ...
      final claimSetAudienceN = JwtClaim(
        audience: <String>[audience1, audience2, audience3],
      );
      ...
      test('Audience not found', () {
        ...
        expect(
          () => claimSetAudience0.validate(audience: missingAudience),
          returnsNormally,
        );

        expect(
          () => claimSetAudience1.validate(audience: missingAudience),
          throwsA(equals(JwtException.audienceNotAllowed)),
        );

        expect(
          () => claimSetAudienceN.validate(audience: missingAudience),
          throwsA(equals(JwtException.audienceNotAllowed)),
        );
      });
    });
    ...
    group('Time claims', () {
      ...
      group('With NotBefore', () {
        ...
        test('At Expiry: invalid (expired)', () {
          expect(
            () => claimSet.validate(currentTime: expiry),
        ...
        test('Just after Expiry: invalid (expired)', () {
          expect(
            () => claimSet.validate(currentTime: expiry.add(smallDelay)),
            throwsA(equals(JwtException.tokenExpired)),
          );
        });
        ...
        test('Just after Expiry, allowing for clock skew: valid', () {
          claimSet.validate(
            currentTime: expiry.add(smallDelay),
            allowedClockSkew: maxSkew,
          );
        });

        test('Way after Expiry: invalid (expired)', () {
          expect(
            () => claimSet.validate(currentTime: expiry.add(bigDelay)),
            throwsA(equals(JwtException.tokenExpired)),
          );
        });

        test(
          'Way after Expiry, allowing for clock skew: invalid (expired)',
          () {
            expect(
              () => claimSet.validate(
                currentTime: expiry.add(bigDelay),
                allowedClockSkew: maxSkew,
              ),
              throwsA(equals(JwtException.tokenExpired)),
            );
      ...
      group('Redundant time claims', () {
        ...
        test('NotBefore at IssuedAt', () {
          ...
          readyWhenIssuedClaimSet.validate(
            currentTime: expiry.subtract(smallDelay),
          ); // expect no exception
        });
