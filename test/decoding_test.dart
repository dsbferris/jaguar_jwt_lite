...
void main() {
  group('Decoding', () {
    ...
    test('JWS example from RFC 7515', () {
      ...
      final claimSet = decodeAndVerifyHS256(token, hmacKey);
    ...
    test('Malformed claims', () {
      ...
      badTokens.forEach((desc, token) {
        expect(
          () => decodeAndVerifyHS256(token, secret),
    ...
    test('No claims', () {
      ...
      final claimSet = decodeAndVerifyHS256(token, secret);
    ...
    //       expect(
    //         decodeAndVerifyHS256(token, correctSecret),
    ...
    //       expect(
    //         () => decodeAndVerifyHS256(token, wrongSecret),
    ...
    //       assert(
    //         B64urlEncRfc7515.decodeUtf8(parts[0]) == goodHeader,
    //         'assumption about generated JWT header is wrong',
    //       );

    //       // Note: decodeAndVerifyHS256 checks the header values before
    ...
    //           expect(
    //             () => decodeAndVerifyHS256(tamperedToken, correctSecret),
